...
Net::Net(const std::vector<LayerType> &topology, const double &init_range,
         const bool &enable_batch_learning)
    : topology(topology) {
  // create Net
...
Net::Net(const Net *other, const double &init_range,
         const bool &enable_batch_learning)
...
void Net::init(const double &init_range, const bool &enable_batch_learning) {
  if (topology.size() < 2) {
    std::cerr << "Invalid net: Topology-size is to small or 0: "
              << topology.size() << std::endl;
    return;
  }
  // Create Net
  ...
  for (unsigned layerNum = 0; layerNum < topology.size(); ++layerNum) {
    ...
    // speichere Nummer des Nächsten Layers, esseiden, dieser ist der Letzte,
    // dann speichere 0
    unsigned anzahl_an_neuronen_des_naechsten_Layers =
        (layerNum == topology.size() - 1)
            ? 0
            : topology.at(layerNum + 1).neuronCount;

    for (unsigned neuronNum = 0; neuronNum < neuron_count; ++neuronNum) {
      m_layers[layerNum][neuronNum] =
          new Neuron(anzahl_an_neuronen_des_naechsten_Layers, neuronNum,
                     topology.at(layerNum), init_range, enable_batch_learning);
    }
...
Net::~Net() {
  ...
  for (unsigned layerNum = 0; layerNum < topology.size(); ++layerNum) {
    for (unsigned neuronNum = 0;
         neuronNum < topology.at(layerNum).neuronCount + 1 /*BIAS*/;
         ++neuronNum) {
...
bool Net::save_to(const std::string &filename) {
  ...
  if (outputFile.is_open()) {
    ...
    for (unsigned layer = 0;
         layer < layerCount() - 1 /*letzte hat keine verbindungen*/; ++layer) {
      for (unsigned neuron = 0; neuron < neuronCountAt(layer) + 1 /*BIAS*/;
           ++neuron) {
        for (unsigned weight = 0;
             weight < m_layers[layer][neuron]->getConections_count();
             ++weight) {
...
bool Net::load_from(const std::string &filename,
  ...
  if (inputFile.is_open()) {
    ...
    if (!std::getline(inputFile, top_lines[0]) ||
        !std::getline(inputFile, top_lines[1])) {
    ...
    if (getTopology().empty() == false) {
      if (getTopologyStr() != top_lines[0]) {
        std::cerr << "Net already initialised with wrong topology! Init("
                  << getTopologyStr() << ") Given: (" << top_lines[0] << ")!"
                  << std::endl;
        return false;
      } else {
        // Init...
    ...
    // Load weights...
    for (unsigned layer = 0;
         layer < layerCount() - 1 /*letzter Layer braucht keine Gewichte!!!!*/;
         ++layer) {
      for (unsigned neuron = 0;
           neuron < neuronCountAt(layer) + 1 /*BIAS!!!!!!*/; ++neuron) {
        std::string line;
        ...
        if (weights.size() != topology.at(layer + 1).neuronCount) {
          ...
          std::cerr << "Error: Mismatch in weight count: " << weights.size()
                    << " vs " << topology.at(layer + 1).neuronCount
                    << std::endl;
          return false;
        }

        for (unsigned weightIdx = 0; weightIdx < weights.size(); ++weightIdx) {
          m_layers[layer][neuron]->m_outputWeights[weightIdx] =
              weights[weightIdx];
        }
      }
    ...
    for (size_t index = substring.find('_'); index != std::string::npos;
      ...
      if (count == 0) { // Topology
        ...
        if (part_s != "INPUT" && part_s.length() != 3) {
          std::cerr << "Invalid length Aggregationsfunktion : " << part_s
                    << std::endl;
          break;
        } else {
          if (part_s == "SUM")
            ...
            std::cerr << "Invalid Aggregationsfunktion: " << part_s
                      << std::endl;
            break;
          }
        }
        ...
        if ((part_s != "LAYER" && part_s != "IDENTITY" &&
             part_s != "SOFTPLUS" && part_s != "LEAKYRELU" &&
             part_s != "SIGMOID") &&
            part_s.length() != 4) {
          ...
          if (part_s == "TANH")
            ...
            std::cerr << "Invalid Aktivierungsfunktion: " << part_s
                      << std::endl;
            break;
          }
        }
        ...
        std::cerr << "Invalid Input - More than 2 x '-': " << substring
                  << std::endl;
        break;
      }
...
std::string Net::getTopologyStr() const {
  ...
  for (unsigned i = 0; i < getTopology().size(); ++i)
    retV += std::to_string(getTopology().at(i).neuronCount) + "_" +
            getTopology().at(i).aggregationsfunktionToString() + "_" +
            getTopology().at(i).aktivierungsfunktionToString() +
            ((i != getTopology().size() - 1) ? "," : "");
  return retV;
}

bool Net::createCopyFrom(const Net *origin) {
  if (topology.size() != origin->topology.size()) {
    std::cerr << "topology.size() != origin->topology.size(): "
              << topology.size() << " != " << origin->topology.size()
              << std::endl;
    return false;
  }
  ...
  for (unsigned layerNum = 0; layerNum < topology.size(); ++layerNum) {
    ...
    if (topology.at(layerNum).neuronCount !=
        origin->topology.at(layerNum).neuronCount) {
      std::cerr << "topology.at(layerNum).neuronCount != "
                   "origin->topology.at(layerNum).neuronCount: "
                << topology.at(layerNum).neuronCount
                << " != " << origin->topology.at(layerNum).neuronCount
                << std::endl;
      return false;
    }
    ...
    for (unsigned neuronNum = 0;
      ...
      if (m_layers[layerNum][neuronNum]->getConections_count() !=
          origin->m_layers[layerNum][neuronNum]->getConections_count()) {
        std::cerr
            << "m_layers[layerNum][neuronNum]->getConections_count() != "
               "origin->m_layers[layerNum][neuronNum]->getConections_count(): "
            << m_layers[layerNum][neuronNum]->getConections_count() << " != "
            << origin->m_layers[layerNum][neuronNum]->getConections_count()
            << std::endl;
      ...
      for (unsigned c = 0;
           c < this->m_layers[layerNum][neuronNum]->getConections_count();
           c++) {
        this->m_layers[layerNum][neuronNum]->m_outputWeights[c] =
            origin->m_layers[layerNum][neuronNum]->m_outputWeights[c];
      }
    }
  }
  return true;
  ...
  for (unsigned layerNum = 0; layerNum < topology.size(); ++layerNum) {
    for (unsigned neuronNum = 0;
      ...
      for (unsigned c = 0;
           c < this->m_layers[layerNum][neuronNum]->getConections_count();
           c++) {
        diff += abs((this->m_layers[layerNum][neuronNum]->m_outputWeights[c] -
                     other->m_layers[layerNum][neuronNum]->m_outputWeights[c]));
        w_count++;
      }
    }
  }
...
std::vector<LayerType> Net::getTopology() const { return topology; }

void Net::mutate(const double &mutation_rate, const double &mutation_range) {
  for (unsigned layerNum = 0; layerNum < topology.size() - 1 /**???*/;
       ++layerNum) {
    for (unsigned neuronNum = 0;
         neuronNum < topology.at(layerNum).neuronCount + 1 /*bias*/;
         ++neuronNum) {
...
double Net::getConWeight(const unsigned int &layer,
                         const unsigned int &neuronFrom,
                         const unsigned int &neuronTo) const {
  if (layer >= layerCount() ||
      neuronFrom >= neuronCountAt(layer) + 1 /*bias*/ ||
      neuronTo >= m_layers[layer][neuronFrom]->getConections_count()) {
    std::cout << "getConWeight() invalid: " << layer << " " << neuronFrom << " "
              << neuronTo << std::endl;
    return -1.0;
  }
...
double Net::getNeuronValue(const unsigned int &layer,
                           const unsigned int &neuron) const {
  if (layer >= layerCount() || neuron >= neuronCountAt(layer) + 1 /*bias*/) {
    std::cout << "getNeuronValue() invalid: " << layer << " " << neuron
              << std::endl;
    return -1.0;
  }
...
unsigned int Net::layerCount() const { return getTopology().size(); }

unsigned int Net::neuronCountAt(const unsigned int &layer) const {
  return getTopology().at(layer).neuronCount;
}

double Net::recentAverrageError() const { return m_recentAverrageError; }
...
void Net::feedForward(const double *input) {
  ...
  for (unsigned layerNum = 1; layerNum < layerCount(); ++layerNum) {
    ...
    bool is_softmax =
        thisLayer[0]->getAktiF() == LayerType::Aktivierungsfunktion::SMAX;
    double maxActivation =
        -std::numeric_limits<double>::infinity(); // Initialize maxActivation to
                                                  // negative infinity
    ...
    if (is_softmax) {
      for (unsigned n = 0; n < neuronCountAt(layerNum); ++n)
        maxActivation = std::max(maxActivation,
                                 thisLayer[n]->getSumme_Aggregationsfunktion());
      for (unsigned n = 0; n < neuronCountAt(layerNum); ++n)
        logSumExp += std::exp(thisLayer[n]->getSumme_Aggregationsfunktion() -
                              maxActivation);
...
void Net::backProp(double *targetVals, const double &eta, const double &alpha,
                   const bool &batchLearning) {
  Layer &outputLayer = m_layers[this->layerCount() - 1]; // start from behind !
  unsigned output_neuron_count_ohne_bias =
      neuronCountAt(this->layerCount() - 1) /*MACH ICHWEG!!!!! - 1*/;
  m_error = 0.0;
  ...
  m_recentAverrageError =
      (m_recentAverrageError * m_recentAverangeSmoothingFactor + m_error) /
  ...
  // Calculate gradients on hidden layers: starts with max -2, because max -1 is
  // last and wee dont need the output layer, so -2, and stop with 1: no inputl
  for (unsigned long Layernum = layerCount() - 2; Layernum > 0; --Layernum) {
    ...
    for (unsigned n = 0; n < neuronCountAt(Layernum) + 1; ++n) {
      hidenLayer[n]->calcHiddenGradients(nextLayer,
...
void Net::applyBatch() {
  // gewichte anpassen
  for (long Layernum = layerCount() - 1 - 1 /*skip last*/;
       Layernum >= /*update also first layer*/ 0; --Layernum) {
    Layer &layer = m_layers[Layernum];
    for (unsigned n = 0; n < neuronCountAt(Layernum) + 1; ++n) {
      layer[n]->applyBatch();
    }
